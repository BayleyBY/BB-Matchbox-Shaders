...
- Every colour-science shader supports the same five spaces with the same `colorSpace` int-popup enum: `0=Rec.709, 1=Scene-Linear, 2=ACEScg, 3=ACEScct, 4=ARRI LogC4` (XML popup titles: "Rec.709", "Scene-Linear", "ACEScg", "ACEScct", "ARRI LogC4"). The XML `PopupEntry Value`s must stay in sync with the `if (colorSpace == N)` chains in that shader's GLSL — keep this five-space set intact when adding a new colour shader.
- Working-space strategy varies by what the shader's model needs (each GLSL documents its own): the density/perceptual shaders operate in place for log inputs and gamma-encode linear inputs to γ2.4; the linear-light shaders (spectral, pigment, CAM16, scotopic) decode to linear and matrix-convert AP1/AWG4 primaries to their Rec.709 working space (`AP1_TO_709`/`AWG4_TO_709` + exact inverses, Bradford D60→D65-adapted AP1 pair so ACES neutrals stay neutral); BB_FilmCoupler decodes transfer only (its per-channel coupling acts on native layers); BB_WhiteBalance folds primaries into its Bradford cone matrices (`A_AP1`/`A_AWG4`).
- Shared transfer snippets: `acescct2lin`/`lin2acescct` and `logc42lin`/`lin2logc4` (LogC4 EI800 constants folded from the 2022 spec) are copied identically across shaders — the `mix()`+`step()` branchless form needs the `max()` NaN guard on `log2` because both branches evaluate.
