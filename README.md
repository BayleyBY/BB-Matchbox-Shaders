...
### BB_SpectralFilter
Applies a lighting **gel / filter spectrally** — the transmission sibling of BB_SpectralWB. It reconstructs a reflectance spectrum per pixel, integrates it with and without the filter transmission against the CIE observer, and applies the ratio, so **different colours respond by different, spectrally-correct amounts** (a channel-mixer applies one global rule). Filters: CTB (cool), CTO (warm), Plus/Minus Green, with Strength and Amount. All spectra are analytic and evaluated in a bounded loop — no arrays or dynamic indexing, so it stays GLSL-1.20-safe. Rec.709 / Scene-Linear; identity when Filter = None. Honest: analytic gel curves, not measured Lee/Rosco data.
